...
from stvid.camera.metadata import add_camera_frame_metadata
from stvid.camera.shared import allocate_double_frame_buffer
import logging
import configparser
import argparse
...
def compress(image_queue, z1base, t1base, z2base, t2base, nx, ny, nz, tend, path, device_id, conf_file, gentl_buffers=None, free_buffer_queue=None):
    ...
    if gentl_buffers is None:
        ...
        gentl_views = tuple(buffer.map() for buffer in gentl_buffers)
        z1 = z2 = t1 = t2 = None
    ...
    try:
        ...
        while True:
            ...
            # Select completed capture buffer
            camera_metadata = None

            if gentl_views is None:
                if proc_buffer == 1:
                    t = t1
                    z = z1
                elif proc_buffer == 2:
                ...
                view = gentl_views[proc_buffer - 1]
                z = view.image
                t = view.host_timestamps_ns.astype(np.float64) / 1e9
                camera_metadata = view
            ...
            if camera_metadata is not None:
                add_camera_frame_metadata(
                    hdr,
                    camera_timestamps=camera_metadata.camera_timestamps,
                    camera_timestamp_valid=(
                        camera_metadata.camera_timestamp_valid
                    ),
                    frame_ids=camera_metadata.frame_ids,
                    frame_id_valid=camera_metadata.frame_id_valid,
                    camera_timestamp_frequency_hz=(
                        camera_metadata.camera_timestamp_frequency_hz
                    ),
                    camera_timestamp_source=(
                        camera_metadata.camera_timestamp_source
                    ),
                )
